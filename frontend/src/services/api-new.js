...
export const torneoAPI = {
    ...
    create: async (torneoData, loginUser = null, loginPass = null) => {
        const config = loginUser && loginPass 
            ? createAuthConfig(loginUser, loginPass, 'POST', torneoData)
            : createApiConfig('POST', torneoData);
        const response = await fetch(`${TORNEO_API_URL}/torneos/`, config);
        return handleResponse(response);
    },
    ...
    update: async (id, torneoData, loginUser = null, loginPass = null) => {
        const config = loginUser && loginPass 
            ? createAuthConfig(loginUser, loginPass, 'PUT', torneoData)
            : createApiConfig('PUT', torneoData);
        const response = await fetch(`${TORNEO_API_URL}/torneos/${id}/`, config);
        return handleResponse(response);
    },
    ...
    activar: async (id, loginUser = null, loginPass = null) => {
        ...
        const response = await fetch(`${TORNEO_API_URL}/torneos/${id}/activar/`, config);
        return handleResponse(response);
    },
    ...
    finalizar: async (id, loginUser = null, loginPass = null) => {
        const config = loginUser && loginPass 
            ? createAuthConfig(loginUser, loginPass, 'POST')
            : createApiConfig('POST');
        const response = await fetch(`${TORNEO_API_URL}/torneos/${id}/finalizar/`, config);
        return handleResponse(response);
    },
};
